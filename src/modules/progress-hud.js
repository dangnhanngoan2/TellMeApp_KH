...
export function setLoading (loading: boolean = true) {
  const PHUD = new ProgressHUD()
  loading ? PHUD.showSpinIndeterminate() : PHUD.dismiss()
}

export function setTextLoading (loading: boolean = true) {
  const PHUD = new ProgressHUD()
  loading ? PHUD.showSimpleText('hihihihihi', 2000) : PHUD.dismiss()
}
