...
console.log('FusedLocation:1 ', FusedLocation)
    ...
    : {
      getFusedLocation: forceNewLocation =>
        forceNewLocation
          ...
          : FusedLocation.getFusedLocation(false),
      startLocationUpdates: FusedLocation.startLocationUpdates,
      stopLocationUpdates: FusedLocation.stopLocationUpdates,
      on: (eventName, cb) => {
        if (eventNames.indexOf(eventName) === -1) {
          throw new Error(
            "Event name has to be one of 'fusedLocation' or 'fusedLocationError'"
          )
        }
        return {
          listener: DeviceEventEmitter.addListener(eventName, cb).listener,
          eventName
        }
      },
      off: subscription =>
      ...
      getConstants: FusedLocation.getConstants,
      Constants: {
        HIGH_ACCURACY: 0,
        BALANCED: 1,
        LOW_POWER: 2,
